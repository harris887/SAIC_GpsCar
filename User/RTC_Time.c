...
void RTC_Configuration(void)
{
	...
	RCC_RTCCLKConfig(RCC_RTCCLKSource_LSE);
        ...
        //RCC_RTCCLKConfig(RCC_RTCCLKSource_HSE_Div128);/*新时钟-Harris错误掉电不行*/
	...
	RTC_SetPrescaler(32767);/*新分配系数*///(8000000/128)-1
...
u8* ConvertDateToAscii(struct tm time_now)
{
    static u8 DATE[11]={0};
    u8 i=0;
    DATE[i++]=((time_now.tm_year%10000)/1000)+48;//千年
    DATE[i++]=((time_now.tm_year%1000)/100)+48;//百年
    DATE[i++]=((time_now.tm_year%100)/10)+48;//十年
    DATE[i++]=((time_now.tm_year%10)/1)+48;//个年
    DATE[i++]='-';
    DATE[i++]=(((time_now.tm_mon+1)%100)/10)+48;//十月
    DATE[i++]=(((time_now.tm_mon+1)%10)/1)+48;//个月
    DATE[i++]='-';
    DATE[i++]=((time_now.tm_mday%100)/10)+48;//十月
    DATE[i++]=((time_now.tm_mday%10)/1)+48;//个月
    DATE[i++]=0;/*字符串结束*/
    return DATE;
}

u8* ConvertTimeToAscii(struct tm time_now)
{
    static u8 time[9]={0};
    u8 i=0;
    time[i++]=((time_now.tm_hour%100)/10)+48;//十小时
    time[i++]=((time_now.tm_hour%10)/1)+48;//个小时
    time[i++]=':';
    time[i++]=((time_now.tm_min%100)/10)+48;//十分
    time[i++]=((time_now.tm_min%10)/1)+48;//个分
    time[i++]=':';
    time[i++]=((time_now.tm_sec%100)/10)+48;//十秒
    time[i++]=((time_now.tm_sec%10)/1)+48;//个秒
    time[i++]=0;
    return time;
}
...
{
#if 0
static u8 date_str[12]={0};
static u8 time_str[9]={0};  
    u8 i=0;
    struct tm time_now;
#define date  date_str
#define time  time_str
    ...
    if(TimeDisplay)
    {
      ...
      TimeDisplay=0;
      time_now=Time_GetCalendarTime();
      LCD_Print(2,1,ConvertDateToAscii(time_now));
      LCD_Print(3,2,ConvertTimeToAscii(time_now));
    }    
#endif
}
